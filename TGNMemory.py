...
from dataset import UserLocationInteractionDataset
...
data = dataset[0].to(device)  # Move entire data to GPU for speed
assert data.t.is_cuda, "data.t is not on GPU"
assert data.msg.is_cuda, "data.msg is not on GPU"

# Split data and initialize data loaders with negative sampling
...
# TensorBoard writer
writer = SummaryWriter(log_dir='runs/TGN_Training')
...
# --- Evaluation function (Val/Test) ---
@torch.no_grad()
def test(loader, split='Val', epoch=0):
    memory.eval()
    gnn.eval()
    link_pred.eval()
...
for epoch in range(1, 51):
    ...
    val_ap, val_auc = test(val_loader, 'Val', epoch)
    test_ap, test_auc = test(test_loader, 'Test', epoch)
